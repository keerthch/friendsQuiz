...
const Challenge = ({ navigation }: Props) => {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `https://cxn8d58dd1.execute-api.ap-south-1.amazonaws.com/prod?level=${150}&name=${name}`
      );
  ...
  const fetchQuestions = async () => {
    try {
      setIsVisible(true);
      const response = await fetch(
        `https://cxn8d58dd1.execute-api.ap-south-1.amazonaws.com/prod?level=${200}&name=${name}`
      );
