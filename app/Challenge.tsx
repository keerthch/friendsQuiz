...
const Challenge = ({ navigation }: Props) => {
  ...
  useEffect(() => {
    ...
    const scheduleNotificationOnce = async () => {
      try {
        const hasScheduled = await AsyncStorage.getItem("notificationScheduled");
        if (!hasScheduled) {
          // Schedule the notification
          await Notifications.scheduleNotificationAsync({
            content: {
              title: "IPL Weekly Challenge is LIVE! 🏏",
              body: "Win big prizes! Join now to compete and climb the leaderboard! 🏆",
            },
            trigger: {
              weekday: 1,       // Saturday (1 = Sunday, 7 = Saturday)
              hour: 14,         // 4 PM
              minute: 15,
              repeats: true,    // Ensures it repeats weekly
            } as Notifications.CalendarTriggerInput, // Cast explicitly to CalendarTriggerInput
          });

          // Mark as scheduled
          await AsyncStorage.setItem("notificationScheduled", "true");
          console.log("Notification scheduled for the first time.");
        } else {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `https://5jw0kp4zk5.execute-api.ap-south-1.amazonaws.com/prod?level==${150}&name=${name}`
      );
  ...
  const fetchQuestions = async () => {
    try {
      setIsVisible(true);
      const response = await fetch(
        `https://5jw0kp4zk5.execute-api.ap-south-1.amazonaws.com/prod?level=${200}&name=${name}`
      );
